...
class StateMachine2[S: Enum, E: Enum, C](StateMachineMixin[S, E, C]):
    def __init__(
    ...
    def _state_transition(self, event: E | None, context: C) -> tuple[S, S | None]:
    ...
    def _execute_on_entry(self, context: C) -> None:
        self._run_actions(
            context=context,
            actions=self._config.on_entry.get(self._state),
    ...
    def _execute_on_exit(self, context: C) -> None:
        self._run_actions(
            context=context,
            actions=self._config.on_exit.get(self._state),
